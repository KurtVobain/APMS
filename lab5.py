...
def init_centroids(data, k):
    ...
    # ВАШ КОД ЗДЕСЬ
    result = random.sample(data, k)  # список k элементов
    # =============
...
def find_closest_centroid(value, centroids):
    ...
    # ВАШ КОД ЗДЕСЬ
    evcl = np.sqrt(np.sum(pow(np.array(centroids) - np.array(value), 2), axis = 1))
    closest_index = np.argmin(evcl)
    # =============
...
def compute_cluster_center(cluster_data):
    ...
    # ВАШ КОД ЗДЕСЬ
    result = np.sum(cluster_data, axis = 0)/len(cluster_data)
    # =============
...
def kmeans(data, k):
    ...
    while True:
        ...
        c = []
        for i in range(k):
            c.append(compute_cluster_center(clusters[i]))
